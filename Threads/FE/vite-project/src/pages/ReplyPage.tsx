    ...
    <Center py={2}>
      <Box
        ...
        <Box mt="3" borderRadius={10}>
          <form action="" onSubmit={handlePostReply}>
          <Box display="flex" alignItems= "center" justifyContent= "space-between">
            <Box display="flex" alignItems= "center" justifyContent= "space-between">
              ...
              <Input name='content' onChange={handleChange} borderStyle="none" color="black" type="text" placeholder="Reply..." />
            ...
            <Button type="submit" w="15%" size="sm" bgColor="#52616B" color="white" borderRadius="20px" ml="2" _hover={{bg: 'blue.500',}}>Post</Button>
          </Box>
          </form>
        </Box>
        {replies?.map((reply) => {
          return (
        <Stack mt={0} direction={'row'} spacing={4} align={'center'} mb={'2'}>
          <Avatar src={reply.user?.picture} mb={10}/>
          <Box>
            <Stack direction={'row'} spacing={3} fontSize={'sm'} display={'flex'} mb={'5px'} mt={5}>
                <Text fontWeight={600}>{reply.user?.full_name}</Text>
        ...
        direction={'row'}
             ...
             <Button color="#1E2022" variant='ghost' leftIcon={<BiLike color={"red"} marginRight={"5px"}/>}>Like</Button>
             <Button color="#1E2022" flex='1' variant='ghost' leftIcon={<BiChat/>}> Replies</Button>
        </Stack>
          </Box>
        </Stack>
        )
