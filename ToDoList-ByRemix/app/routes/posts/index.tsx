...
export async function loader({request}: LoaderArgs) {
  ...
  return await getPosts(searchTitle)
}
...
export async function action({ request }: ActionArgs) {
    ...
    if (actionType === "update" && postId && updateTitle) {
        const updated = await updatePost(parseInt(postId as string), updateTitle as string);
        return (
...
export default function PostsIndex() {
    ...
    const [formUpdate, setFormUpdate] = useState<{ [id: number]: boolean }>({});

    const toggleButtonUpdate = (id: number) => {
          ...
          <Flex justifyContent={'center'}>
            <div>
              {state === 'loading' ? (
                ...
                <Box borderRadius={'10'} w={'550px'} m={10} boxShadow={'lg'} p={'20px 50px'} border={'1px solid white'}>
                  <Text fontWeight={'bold'} fontSize={'25px'} display="flex" justifyContent={'center'} mb={8}>
                    To Do List
                  </Text>
                  <Flex justifyContent={'center'} mb={8}>
                    <Form method="get">
                      <Input type="text" w={'400px'} name="searchTitle" placeholder="Search your plan.." h={'25px'} p={4}></Input>
                    </Form>
                    <Button fontSize={'20px'} onClick={handleBack} type="submit" variant={"ghost"} pb={2}>🔄️</Button>
                  </Flex>
                  {data.map((d) => (
                    <Stack display={'flex'} justifyContent={'center'}  borderBottom={'1px solid white'} mb={4}>
                      <Text key={d.id}>
                        <Flex alignItems={'center'} h={'20px'} mb={3} ml={5} justifyContent={'space-between'}>
                          <Text>{d.title}</Text>
                          <Form method="PATCH">
                        ...
                        <Flex justifyContent={'space-around'} alignItems={'center'} mb={1} w={'40%'} ml={'1'}>
                          {formUpdate[d.id] ? (
                            <Form method="post">
                              ...
                              bgColor={'red.500'}
                              borderStyle={'none'}
                              _hover={{ bgColor: 'green.500', transform: 'translateY(-2px)' }}>Cancel</Button>
                            </Form>
                          ) : ( 
                            <>
                              <Button
                        ...
                        <Text fontSize="11px" ml={'6'} mb={3}> {moment(d.createAt).startOf("minute").fromNow()}</Text>
                      </Text>
                    </Stack>
                  ))}
                </Box>
